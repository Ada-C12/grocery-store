...
describe "Customer Wave 3" do
  ...
  customers_arr = CSV.read(customers_csv)
  
  describe "Customer.save" do
    it "matches all the information of original csv file" do
      Customer.save('new_customer_list')
      new_customers_arr = CSV.read('new_customer_list')
      
      new_customers_arr.each_with_index do |customer, line|
        customer.each_with_index do |info, index|
          expect(info).must_equal customers_arr[line][index]
        end
      end
