...
describe "Order Wave 1" do
  ...
  # Tests for Wave 1 Optional Enhancements
  describe "remove product" do
    ...
    it "raises ArgumentError if trying to remove product that is not there" do
      products = { "banana" => 1.99, "cracker" => 3.00 }
      
      order = Order.new(1337, products, customer)
      
      expect {
        order.remove_product("coffee")
      }.must_raise ArgumentError
    end
...
describe "Order Wave 2" do
  ...
  describe "Order.find" do
    ...
    it "Returns nil for an order that doesn't exist" do
      expect(Order.find(2999)).must_be_nil
    end
  end
  
  describe "Order.find_by_customer" do 
    it "Can find all the orders associated with a customer" do
      orders = Order.find_by_customer(4)
      expect(orders.length).must_equal 3
      expect(orders.include?(11)).must_equal true
      expect(orders.include?(44)).must_equal true
      expect(orders.include?(86)).must_equal true
    end
    
    it "Returns nil for a customer that doesn't exist" do
      expect(Order.find_by_customer(2999)).must_be_nil
    end
  end
