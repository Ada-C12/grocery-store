...
describe "Order Wave 2" do
  ...
  describe "Order.find_by_customer" do
    it "Returns an array of the right length and objects" do
      cust_orders = Order.find_by_customer(21)
      
      expect(cust_orders.length).must_equal 3
      
      cust_orders.each do |o|
        expect(o).must_be_kind_of Order
        expect(o.id).must_be_kind_of Integer
        expect(o.products).must_be_kind_of Hash
        expect(o.customer).must_be_kind_of Customer
    ...
    it "Returns accurate information about customer" do
      id = 34
      products = {
        "Brown Flour" => 16.12,
        "Choy Sum" => 87.67,
      ...
      orders = Order.find_by_customer(7)
      
      # Check that all data was loaded as expected
      orders.each do |o|
        expect(o.id).must_equal id
        expect(o.products).must_equal products
        expect(o.customer).must_be_kind_of Customer
        expect(o.customer.id).must_equal customer_id
        expect(o.fulfillment_status).must_equal fulfillment_status
      end
    end
  end
end
