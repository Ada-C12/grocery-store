class Order
  ...
  def self.all
    orders = CSV.open('data/orders.csv', 'r+').map do |order|
      ...
      Order.new(id.to_i, product_hash, customer, fulfillment_status.to_sym)
    end
    return orders
  end
  ...
  def self.save(file)
    File.open(file, 'a+') do |content|
      self.all.each do |order|
        ...
        content << "#{order.id},#{product_string.delete_suffix(';')},#{order.customer.id},#{order.fulfillment_status}\n"
      end
    end
  end
